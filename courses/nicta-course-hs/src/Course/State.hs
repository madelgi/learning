...
instance Functor (State s) where
  ...
  f <$> s = State $ \x -> (f . fst $ runState s x, x)
...
instance Applicative (State s) where
    ...
    (<*>) s1 s2 = State $ \x -> ((f x) (inner x), x)
          where inner a = fst (runState s2 a)
                f a     = fst (runState s1 a)
...
--   However, while performing the search, we sequence some `Monad` effect
--   through.
--
...
--
-- >>> let p x = (\s -> (const $ pure (x == 'c')) =<<
--                  put (1+s)) =<<
--                  get in runState (findM p $ listh ['a'..'h']) 0
--
-- (Full 'c',3)
--
-- >>> let p x = (\s -> (const $ pure (x == 'i')) =<<
--                  put (1+s)) =<<
--                  get in runState (findM p $ listh ['a'..'h']) 0
--
-- (Empty,8)
...
-- | Find the first element in a `List` that repeats. It is possible that
--   no element repeats, hence an `Optional` result.
--
...
--   eventually reaches 1 after repetition. In contrast, a sad number (not a happy
--   number) is where the sum of the square of its digits never reaches 1 because it
--   results in a recurring sequence.
--
