...
seqOptional ts = foldRight f (P.return Nil) ts
    where f a b = a P.>>= \x -> (b P.>>= \xs -> (P.return (x :. xs)))
...
notReverse = error "lazy, but I'm assuming this is impossible based on phrasing above"
